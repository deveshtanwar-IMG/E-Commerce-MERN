...
const OTPInputGroup = ({email}) => {
    ...
    const handleValidate = () => {
        const otp = inputValues.input1 + inputValues.input2 + inputValues.input3 + inputValues.input4 + inputValues.input5 + inputValues.input6;
        console.log("entered otp: ", otp)
        axios.post('http://localhost:5001/otp-validate', { "email": email, "otp": otp })
            .then((res) => {
                if (res?.data?.success) {
                    window.scrollTo(0, 0)
                ...
                < div className="container height-100 d-flex justify-content-center align-items-center" >
                    <div className="position-relative">
                        <div className="card p-2 text-center">
                            ...
                            <div className="mt-4">
                                <button className="btn btn-danger px-4 validate" onClick={handleValidate}>Validate</button>
                            </div>
                        </div>
                    </div>
                </div >
